...
class IntakeOutcome:
    ...
    # True when the intake tree stopped early because a red-flag rule fired
    # (§10 / Invariant §6 / _maybe_stop_for_confirmed_red_flag), OR the
    # finalized narrative satisfies a rule regardless of when the tree
    # stopped. The routing layer treats this as: issue a RED token now and
    # page a nurse — do not queue behind other patients, do not finish the
    # remaining questions, do not wait for the model.
    needs_immediate_nurse: bool = False
...
class IntakePipeline:
    ...
    @property
    def needs_immediate_nurse(self) -> bool:
        """
        Mid-tree short-circuit. True the moment any answer produces a
        symptom set that satisfies a red-flag rule -- the intake tree has
        already truncated itself; this exposes that fact so the caller can
        issue a RED token and page a nurse right now instead of walking the
        patient through the remaining screens.

        Safe to call after every turn: it is a cheap property read over
        state that _maybe_stop_for_confirmed_red_flag() already set.
        """
        return bool(self.tree_session and self.tree_session.stopped_for_red_flag)
    ...
    def finalize(self) -> IntakeOutcome:
        ...
        # The tree may have truncated early on a red flag (stopped_for_red_flag);
        # or the final narrative may only NOW satisfy a rule that was not yet
        # complete when the last question was answered. Either counts.
        stopped_early = bool(self.tree_session and self.tree_session.stopped_for_red_flag)
        needs_immediate_nurse = red_flag.red_flag or stopped_early
        return IntakeOutcome(
            record=record, red_flag=red_flag,
            needs_immediate_nurse=needs_immediate_nurse,
        )

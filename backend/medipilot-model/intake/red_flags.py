...
def evaluate_all_red_flags(narrative: StructuredNarrative) -> list:
    """
    Every rule the narrative matches, in table order — not just the first.

    evaluate_red_flags() above stays the authority on the single LEADING
    factor shown on the nurse card (§12 Channel 1). This variant exists
    because a patient can genuinely trip more than one rule (chest pain with
    breathlessness AND difficulty speaking in full sentences), and hiding the
    others from the card would understate why the patient is at the front of
    the queue.

    Identical semantics otherwise: deterministic, no model inference, closed
    ObservationCode vocabulary only. Returns [] when nothing matches.
    """
    symptoms = set(narrative.symptoms) if narrative else set()
    results = []
    for rule in RED_FLAG_RULES:
        if _matches(rule, symptoms):
            results.append(
                RedFlagResult(
                    red_flag=True,
                    rule_id=rule.rule_id,
                    matched_observations=sorted((rule.requires_all | rule.requires_any) & symptoms),
                    description=rule.description,
                )
            )
    return results

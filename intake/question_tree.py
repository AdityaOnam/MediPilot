...
CATEGORIES = [
    ...
    ComplaintCategory(
        ...
        symptom_codes=(),
        questions=(
            QuestionNode("mechanism", "How did the injury happen?", AnswerKind.FREE_TEXT),
...
_CATEGORY_KEYWORDS = {c.name: c.keywords for c in CATEGORIES}
...
def classify_complaint(narrative: StructuredNarrative, classifier: Optional[ComplaintClassifier] = None) -> str:
    ...
    if text:
        try:
            ...
            pass  # fall through to the keyword layer below

    text_lower = text.lower().replace("'", "")
    for category in CATEGORIES:
        if category.keywords and any(kw in text_lower for kw in category.keywords):
            return category.name
    return "generic"
